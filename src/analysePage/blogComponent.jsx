...
const BlogComponent = ({ t, posts, currentLanguage }) => {
  const [openTab, setOpenTab] = useState('blog');
  const [tab, setTab] = useState('couples');
  const [selectedPost, setSelectedPost] = useState(null);

  // Function to truncate the post description to the first 5-6 words
  const truncateDescription = (description, wordLimit) => {
    const words = description.split(' ');
    if (words.length <= wordLimit) return description; // Return the full description if it's short enough
    return words.slice(0, wordLimit).join(' ') + '...'; // Otherwise, return truncated description with "..."
  };

  return (
    <div>
      {selectedPost ? (
        <ReadBlog image={selectedPost.image} title={selectedPost.title} description={selectedPost.content} t={t} posts={posts} currentLanguage={currentLanguage} />
      ) : (
              ...
              {posts.length === 0 ? (
                ...
                posts.map((post, index) => (
                  ...
                  >
                    ...
                    />
                    <div className="pr-8">
                      <h4 className="text-lg text-secondary font-semibold">{post.title || t("postTitle")}</h4>
                      <p className="text-sm text-[#006D81]">
                        {truncateDescription(post.content || t("postDescription"), 6)} {/* Limit description to 6 words */}
                        {post.content && post.content.split(' ').length > 6 && (
                          <span
                            className="text-secondary cursor-pointer ml-2"
                            onClick={() => setSelectedPost(post)} // Clicking "Read more" opens the full post
                          >
                            {t("readMore")}
                          </span>
                        )}
          ...
          <section className="flex flex-col gap-6">
            ...
            {tab === 'couples' && (
              <div className="flex flex-col gap-[60px] md:flex-row">
                {[...Array(2)].map((_, index) => (
                  <div key={index} className="flex gap-3 rounded-[5%]">
                    <img
                      src={blogImage}
                      alt="pic"
                      width={90}
